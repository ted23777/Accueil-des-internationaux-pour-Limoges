        ...
        <?php
            if(isset($_POST['chercheranonc'])){
                ...
                if($_POST['colocation']==1){
                    if($_POST['meublée']==1){
                        $rech= $conn->prepare("SELECT * FROM annonce WHERE Université=? || Prix<=? || Prix>=? || Type=? || Surface<=? || Surface>=? || N_chambre=? || Logement_meublé=? || N_colocation_possible=?");
                        ...
                        $rech= $conn->prepare("SELECT * FROM annonce WHERE Université=? || Prix<=? || Prix>=? || Type=? || Surface<=? || Surface>=? || N_chambre=? || N_colocation_possible=?");
                    $rech->execute(array($_POST['v'],$_POST['u'],$_POST['pma'],$_POST['pmi'],$_POST['t'],$_POST['sma'],$_POST['smi'],$_POST['ch'],$_POST['colocation']));
                    }
                }else{
                   if($_POST['meublée']==1){
                        $rech= $conn->prepare("SELECT * FROM annonce WHERE   Université=? || Prix<=? || Prix>=? || Type=? || Surface<=? || Surface>=? || N_chambre=? || Logement_meublé=?");
                        $rech->execute(array($_POST['v'],$_POST['u'],$_POST['pma'],$_POST['pmi'],$_POST['t'],$_POST['sma'],$_POST['smi'],$_POST['ch'],$_POST['meublée']));
                   }else{
                    $rech= $conn->prepare("SELECT * FROM annonce WHERE  Université=? || Prix<=? || Prix>=? || Type=? || Surface<=? || Surface>=? || N_chambre=?");
